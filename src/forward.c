...
static int forward_to_referrals(struct frec *forward, time_t now);
...
static void set_outgoing_mark(struct frec *forward, int fd)
{
  ...
  if (get_incoming_mark(&forward->frec_src.source, &forward->frec_src.dest, 0, &mark))
    if (setsockopt(fd, SOL_SOCKET, SO_MARK, &mark, sizeof(unsigned int)) == -1)
      my_syslog(LOG_WARNING, _("failed to set SO_MARK: %s"), strerror(errno));
}
#endif
...
static void log_query_mysockaddr(unsigned int flags, char *name, union mysockaddr *addr, char *arg, unsigned short type)
{
  ...
  if (addr->sa.sa_family == AF_INET)
	...
	type = ntohs(addr->in.sin_port);
      a.addr4 = addr->in.sin_addr;
      log_query(flags | F_IPV4, name, &a, arg, type);
    }
  else
    {
      if (flags & F_SERVER)
	type = ntohs(addr->in6.sin6_port);
      a.addr6 = addr->in6.sin6_addr;
      log_query(flags | F_IPV6, name, &a, arg, type);
    }
}
...
#define BITS_IN_INT (sizeof(unsigned int) * 8)
	  ...
	  src->udp_pkt_size = (replylimit > USHRT_MAX) ? USHRT_MAX : (unsigned short)replylimit;
  ...
  if (!forward)
      ...
      if (!(forward->stash = blockdata_alloc((char *)header, plen)))
	{
      ...
      forward->recursive_iteration = 0;
      forward->referral_servers = NULL;
      if (option_bool(OPT_RECURSIVE))
	{
	  struct referral_server *cached;
	  forward->flags |= FREC_RECURSIVE;

	  cached = deleg_cache_lookup(daemon->namebuff, now);
	  if (cached)
	    {
	      forward->referral_servers = cached;
	      forward->recursive_iteration = 1;
	    }
	}
  ...
  if ((forward->flags & FREC_RECURSIVE) && forward->referral_servers && !old_src)
    {
      if (forward_to_referrals(forward, now))
	{
	  daemon->metrics[METRIC_DNS_QUERIES_FORWARDED]++;
	  return;
	}
      free_referral_servers(forward->referral_servers);
      forward->referral_servers = NULL;
      forward->recursive_iteration = 0;
    }

  if (forward->flags & FREC_RECURSIVE)
    header->hb3 &= ~HB3_RD;
...
static size_t decompress_rdata(struct dns_header *header, size_t plen,
			       unsigned short type, unsigned char *rdata,
			       unsigned short rdlen, unsigned char *out,
			       size_t outlen)
{
  ...
  switch (type)
	...
	return 0;
      op = do_rfc1035_name(op, name, (char *)olimit);
      if (!op || op + 1 >= olimit) return 0;
      *op++ = 0;
      return (size_t)(op - out);

    case T_MX:
      if (rdlen < 3 || op + 2 >= olimit) return 0;
      memcpy(op, p, 2);
      op += 2; p += 2;
      if (!extract_name(header, plen, &p, name, EXTR_NAME_EXTRACT, 0))
	return 0;
    ...
    default:
      if ((size_t)rdlen > outlen) return 0;
      if (rdlen > 0)
	memcpy(out, rdata, rdlen);
      return (size_t)rdlen;
    }
}
...
static int forward_to_referrals(struct frec *forward, time_t now)
{
  ...
  if (!forward->stash || !forward->referral_servers)
    return 0;
  ...
  if (!option_bool(OPT_NO_0x20) && option_bool(OPT_DO_0x20))
    extract_name(header, plen, NULL, (char *)&forward->frec_src.encode_bitmap, EXTR_NAME_FLIP, 1);
  ...
  for (rs = forward->referral_servers; rs; rs = rs->next)
    {
      ...
      dummy_serv.source_addr.sa.sa_family = rs->addr.sa.sa_family;

      if ((fd = allocate_rfd(&forward->rfds, &dummy_serv)) != -1)
	{
	  while (retry_send(sendto(fd, (char *)header, plen, 0,
				   &rs->addr.sa, sa_len(&rs->addr))));
	  if (errno == 0)
	    {
	      sent++;
	      log_query_mysockaddr(F_SERVER | F_FORWARD, daemon->namebuff,
  ...
  if (sent)
    {
      forward->forward_timestamp = dnsmasq_milliseconds();
      forward->forward_delay = daemon->fast_retry_time;
    }
				 ...
				 int ns_count, const char *zone, time_t now)
{
  ...
  if (daemon->async_ns_in_flight >= daemon->iterative_async_limit)
    {
      if (option_bool(OPT_LOG))
	my_syslog(LOG_INFO, _("recursive: async NS limit reached (%d/%d), falling back to sync"),
		  daemon->async_ns_in_flight, daemon->iterative_async_limit);
      return 0;
    }

  parent->async_ns_count = (ns_count > 4) ? 4 : ns_count;
...
static void async_ns_complete(struct frec *sub, struct dns_header *header, ssize_t n, time_t now)
{
...

  {
    ...
    if (next < parent->async_ns_count)
      {
	char remaining_names[4][MAXDNAME];
	int remaining_count = parent->async_ns_count - next;
	int k;

	for (k = 0; k < remaining_count; k++)
	  safe_strncpy(remaining_names[k], parent->async_ns_names[next + k], MAXDNAME);

	if (resolve_ns_name_async(parent, remaining_names, remaining_count,
				  parent->async_ns_zone, now))
	  return;
      }

    if (option_bool(OPT_LOG))
      my_syslog(LOG_INFO, _("recursive: async NS resolution failed, trying sync fallback"));
...

    {
      ...
      my_syslog(LOG_WARNING, _("recursive: all NS resolution attempts failed, returning SERVFAIL"));
      return_reply(now, parent, err_hdr, parent->stash_len, STAT_OK);
    }
  }
}
...
#  endif
  ...
  if (option_bool(OPT_WIPE_DNSSEC))
    {
      rrfilter(header, &n, RRFILTER_WIPE_DNSSEC);
      header->hb4 &= ~HB4_AD;
    }
  ...
  if (server && RCODE(header) == NXDOMAIN)
...
#ifdef HAVE_DUMPFILE
  if ((STAT_ISEQUAL(status, STAT_BOGUS) || STAT_ISEQUAL(status, STAT_ABANDONED)) && forward->sentto)
...
void reply_query(int fd, time_t now)
{
  ...
  if ((forward->flags & FREC_RECURSIVE) && forward->referral_servers)
    {
	...
	return;
      ...
      if (!recursive_validate_source(forward, &serveraddr))
	return;
	...
	return;
      ...
      if (is_referral(header, (size_t)n))
	{
	  struct referral_server *new_servers;

	  if (forward->recursive_iteration >= MAX_RECURSIVE_ITERATIONS)
	    {
	      my_syslog(LOG_WARNING, _("recursive: too many iterations for %s, returning SERVFAIL"),
			daemon->namebuff);
	      SET_RCODE(header, SERVFAIL);
	      header->ancount = htons(0);
	      header->nscount = htons(0);
...

	  {
	    char zone[MAXDNAME];
	    zone[0] = '\0';
	    referral_zone_name(header, (size_t)n, zone, sizeof(zone));

	    if (zone[0] != '\0')
	      {
		extract_request(header, (size_t)n, daemon->namebuff, NULL, NULL);
		if (strlen(daemon->namebuff) > 0 && strlen(zone) > 0)
		  {
		    size_t nlen = strlen(daemon->namebuff);
		    size_t zlen = strlen(zone);
		    if (!(nlen >= zlen &&
			  (nlen == zlen || daemon->namebuff[nlen - zlen - 1] == '.') &&
			  hostname_isequal(daemon->namebuff + nlen - zlen, zone)))
		      {
			if (option_bool(OPT_LOG))
			  my_syslog(LOG_WARNING, _("recursive: ignoring out-of-bailiwick referral zone '%s' for query '%s'"),
	    ...
	    {
	      unsigned long ref_ns_ttl = 0;
	      new_servers = parse_referral(header, (size_t)n, zone, &ref_ns_ttl);
	      if (!new_servers)
		{
		  char ns_names[4][MAXDNAME];
		  int ns_count, j;

		  ns_count = extract_referral_ns_names(header, (size_t)n, ns_names, 4);

		  if (daemon->iterative_async_limit > 0 &&
		      resolve_ns_name_async(forward, ns_names, ns_count, zone, now))
		    return;
		  ...
		  if (!new_servers)
		    {
		      my_syslog(LOG_WARNING, _("recursive: failed to resolve NS names in referral for %s"),
				daemon->namebuff);
		      SET_RCODE(header, SERVFAIL);
		      header->ancount = htons(0);
		      header->nscount = htons(0);
	      ...
	      if (zone[0] != '\0' && forward->recursive_prev_zone[0] != '\0' &&
		  strlen(zone) <= strlen(forward->recursive_prev_zone))
		{
		  if (option_bool(OPT_LOG))
		    my_syslog(LOG_WARNING, _("recursive: no progress in delegation chain for %s (zone '%s' not deeper than '%s')"),
			      daemon->namebuff, zone, forward->recursive_prev_zone);
		  free_referral_servers(new_servers);
		  SET_RCODE(header, SERVFAIL);
		  header->ancount = htons(0);
		  header->nscount = htons(0);
		  header->arcount = htons(0);
	      ...
	      if (zone[0] != '\0')
		{
		  deleg_cache_store(zone, new_servers, (time_t)ref_ns_ttl);
		  safe_strncpy(forward->recursive_prev_zone, zone, MAXDNAME);
		}
	    }
	  }

	  free_referral_servers(forward->referral_servers);
	  forward->referral_servers = new_servers;
	  forward->recursive_iteration++;

	  if (option_bool(OPT_LOG))
	    my_syslog(LOG_INFO, _("recursive: following referral for %s (iteration %d)"),
		      daemon->namebuff, forward->recursive_iteration);

	  extract_request(header, (size_t)n, daemon->namebuff, NULL, NULL);

	  if (!forward_to_referrals(forward, now))
	    {
	      my_syslog(LOG_WARNING, _("recursive: failed to send to referral servers for %s"),
			daemon->namebuff);
	      SET_RCODE(header, SERVFAIL);
	      header->ancount = htons(0);
	      header->nscount = htons(0);
      ...
      {
	ALIGNED(sizeof(u16)) unsigned char cname_add_synth[2048];

      if ((forward->flags & FREC_RECURSIVE) &&
	  ntohs(header->ancount) > 0 && RCODE(header) == NOERROR)
	{
	  unsigned char *ans_p = skip_questions(header, (size_t)n);
	  if (ans_p)
	    {
	      unsigned short ans_type;
	      int all_cname = 1, j;
	      for (j = 0; j < ntohs(header->ancount) && all_cname; j++)
		{
		  if (!(ans_p = skip_name(ans_p, header, (size_t)n, 10)))
		    break;
		  GETSHORT(ans_type, ans_p);
		  if (ans_type != T_CNAME)
		    all_cname = 0;
		  ans_p += 2 + 4;
		  {
		    unsigned short ans_rdlen;
		    GETSHORT(ans_rdlen, ans_p);
		    if (!ADD_RDLEN(header, ans_p, (size_t)n, ans_rdlen))
		      break;
		  }
		}
	      ...
	      if (all_cname)
		{
		  char cname_target[MAXDNAME];
		  unsigned long cname_ttl = 300;

		  if (forward->cname_chase_count >= 15)
		    {
		      my_syslog(LOG_WARNING, _("recursive: CNAME chain too long for %s, returning SERVFAIL"),
				daemon->namebuff);
		      SET_RCODE(header, SERVFAIL);
		      header->ancount = htons(0);
		      header->nscount = htons(0);
		  ...
		  if (extract_cname_target(header, (size_t)n, cname_target, sizeof(cname_target), &cname_ttl))
		    {
		      struct referral_server *cached;
		      struct cname_chain *entry;
		      unsigned char *query_p;
		      unsigned short qtype;
		      size_t new_qlen;

		      if (option_bool(OPT_LOG))
			my_syslog(LOG_INFO, _("recursive: chasing CNAME %s -> %s"),
		      ...
		      if (!forward->cname_original)
			{
		      ...
		      qtype = forward->cname_original_type;

		      if (qtype == T_A || qtype == T_AAAA)
			{
			  unsigned char *add_p = skip_questions(header, (size_t)n);
			  if (add_p)
			    add_p = skip_section(add_p, ntohs(header->ancount), header, (size_t)n);
			  if (add_p)
			    add_p = skip_section(add_p, ntohs(header->nscount), header, (size_t)n);
			  if (add_p)
			    {
			      struct dns_header *synth = (struct dns_header *)cname_add_synth;
			      unsigned char *sp, *sp_limit;
			      int synth_ancount = 0, k;

			      memset(synth, 0, sizeof(struct dns_header));
			      synth->hb3 = HB3_QR | HB3_RD;
			      synth->hb4 = HB4_RA;
			      synth->qdcount = htons(1);

			      sp = (unsigned char *)(synth + 1);
			      sp_limit = cname_add_synth + sizeof(cname_add_synth);

			      sp = do_rfc1035_name(sp, cname_target, (char *)sp_limit);
			      if (sp && sp + 1 + 4 < sp_limit)
				{
				  *sp++ = 0;
				  PUTSHORT(qtype, sp);
				  PUTSHORT(C_IN, sp);
			      ...
			      for (k = 0; k < ntohs(header->arcount) && sp; k++)
				{
				  char add_name[MAXDNAME];
				  unsigned short add_type, add_class, add_rdlen;
				  unsigned long add_ttl;

				  if (!extract_name(header, (size_t)n, &add_p, add_name, EXTR_NAME_EXTRACT, 10))
				    break;

				  GETSHORT(add_type, add_p);
				  GETSHORT(add_class, add_p);
				  GETLONG(add_ttl, add_p);
				  GETSHORT(add_rdlen, add_p);

				  if (hostname_isequal(add_name, cname_target) &&
				      add_class == C_IN &&
				      add_type == qtype &&
				      ((add_type == T_A && add_rdlen == INADDRSZ) ||
				       (add_type == T_AAAA && add_rdlen == IN6ADDRSZ)) &&
				      CHECK_LEN(header, add_p, (size_t)n, add_rdlen))
				    {
				      unsigned char *name_p = do_rfc1035_name(sp, cname_target, (char *)sp_limit);
				      if (name_p && name_p + 1 + 10 + add_rdlen < sp_limit)
					{
					  *name_p++ = 0;
					  PUTSHORT(add_type, name_p);
					  PUTSHORT(C_IN, name_p);
					  PUTLONG(add_ttl, name_p);
					  PUTSHORT(add_rdlen, name_p);
					  memcpy(name_p, add_p, add_rdlen);
					  name_p += add_rdlen;
					  sp = name_p;
					  synth_ancount++;
					}
				  ...
				  if (!ADD_RDLEN(header, add_p, (size_t)n, add_rdlen))
				    break;
				}

			      if (synth_ancount > 0)
				{
				  synth->ancount = htons(synth_ancount);

				  if (option_bool(OPT_LOG))
				    my_syslog(LOG_INFO, _("recursive: CNAME additional shortcut - %d %s record(s) for %s from Additional section"),
					      synth_ancount, qtype == T_A ? "A" : "AAAA", cname_target);

				  header = synth;
				  n = (ssize_t)(sp - cname_add_synth);
				  goto cname_additional_done;
				}
			    }
			}
			...
			goto recursive_reply;
		      *query_p++ = 0;
		      ...
		      new_qlen = (size_t)(query_p - (unsigned char *)header);
			...
			goto recursive_reply;
		      ...
		      if (!option_bool(OPT_NO_0x20) && option_bool(OPT_DO_0x20))
			extract_name(header, new_qlen, NULL, (char *)&forward->frec_src.encode_bitmap, EXTR_NAME_FLIP, 1);

		      free_referral_servers(forward->referral_servers);
		      forward->referral_servers = NULL;
		      forward->recursive_prev_zone[0] = '\0';
		      forward->recursive_iteration = 0;
		      forward->cname_chase_count++;

		      cached = deleg_cache_lookup(cname_target, now);
		      if (cached)
			{
			  forward->referral_servers = cached;
			  forward->recursive_iteration = 1;
			  ...
			  if (option_bool(OPT_LOG))
			    my_syslog(LOG_INFO, _("recursive: starting resolution for CNAME target '%s'"),
				      cname_target);
			...
			{
			  struct server *s;
			  struct referral_server *root_servers = NULL;
			  int root_count = 0;

			  for (s = daemon->servers; s && root_count < DELEGATION_MAX_ADDRS; s = s->next)
			    {
			  ...
			  if (root_servers)
			    {
			      forward->referral_servers = root_servers;

			      if (option_bool(OPT_LOG))
				my_syslog(LOG_INFO, _("recursive: no delegation cache for CNAME target '%s', starting from root"),
					  cname_target);
    ...
    cname_additional_done:
      if (option_bool(OPT_LOG))
	my_syslog(LOG_INFO, _("recursive: got answer for %s after %d iterations"),
		  daemon->namebuff, forward->recursive_iteration);

      if (forward->cname_chain)
	{
#define MAX_SAVED_RRS 20
#define MAX_RDATA_SIZE 1024
	  struct {
	    char name[MAXDNAME];
	    unsigned long ttl;
	    unsigned short type;
	    unsigned short rdlen;
	    unsigned char rdata[MAX_RDATA_SIZE];
	  } final_rrs[MAX_SAVED_RRS];
	  int rr_count = 0;
	  unsigned char *fp;

	  fp = skip_questions(header, (size_t)n);
	  if (fp)
	    {
	      int j;
	      for (j = 0; j < ntohs(header->ancount) && rr_count < MAX_SAVED_RRS; j++)
		{
		  char rr_name[MAXDNAME];
		  unsigned short rr_type, rr_rdlen;
		  unsigned long rr_ttl;

		  if (!extract_name(header, (size_t)n, &fp, rr_name, EXTR_NAME_EXTRACT, 10))
		    break;
		  GETSHORT(rr_type, fp);
		  fp += 2;
		  GETLONG(rr_ttl, fp);
		  ...
		  if (!ADD_RDLEN(header, fp, (size_t)n, rr_rdlen))
		    break;
		}
	    }

	  {
	    struct dns_header *combined = (struct dns_header *)daemon->packet;
	    ...
	    ansp = skip_questions(combined, forward->cname_original_len);
	    if (!ansp)
	      goto recursive_reply;
	    ...
	    if (option_bool(OPT_CNAME_FLAT))
	      {
		if (option_bool(OPT_LOG))
		  my_syslog(LOG_INFO, _("recursive: CNAME flattening - hiding chain for %s"),
			    forward->cname_chain->source);
	      }
	    else if (option_bool(OPT_CNAME_MINIM))
	      {
		struct cname_chain *last_cc = forward->cname_chain;
		while (last_cc->next)
		  last_cc = last_cc->next;

		{
		  unsigned char *name_start = ansp;

		  ansp = do_rfc1035_name(ansp, forward->cname_chain->source, (char *)limit);
		  if (ansp && ansp + 1 + 10 < limit)
			...
			ansp = do_rfc1035_name(ansp, last_cc->target, (char *)limit);
			if (ansp && ansp + 1 < limit)
			  {
			    *ansp++ = 0;
			    rdlen = ansp - rdata_start;
			    PUTSHORT(rdlen, rdlen_p);
			    anscount++;
			  }
			else
			  ansp = name_start;
		...
		if (option_bool(OPT_LOG))
		  my_syslog(LOG_INFO, _("recursive: CNAME minimization - %s -> %s (skipped %d intermediates)"),
			    forward->cname_chain->source, last_cc->target,
			    forward->cname_chase_count - 1);
	      }
	    else
	      {
		for (cc = forward->cname_chain; cc; cc = cc->next)
		  {
		    ...
		    *ansp++ = 0;
...

		    {
		      ...
		      ansp = do_rfc1035_name(ansp, cc->target, (char *)limit);
		      if (!ansp || ansp + 1 >= limit)
			{
			  ansp = name_start;
			  break;
		...
		*ansp++ = 0;
		...
		ansp += final_rrs[j].rdlen;
		anscount++;
	      }

	    combined->ancount = htons(anscount);

	    if (option_bool(OPT_LOG))
	      my_syslog(LOG_INFO, _("recursive: built CNAME chain response with %d records"), anscount);

	    header = combined;
	    n = (ssize_t)(ansp - (unsigned char *)combined);
	  }
	}
    ...
    recursive_reply:
	  ...
	  && RCODE(header) != SERVFAIL)
	{
	  if (!forward->sentto && daemon->serverarraysz > 0)
	    forward->sentto = daemon->serverarray[0];
	...
	memset(daemon->rr_status, 0, sizeof(*daemon->rr_status) * daemon->rr_status_sz);
      ...
      if (forward->flags & FREC_NS_ASYNC)
	{
	  async_ns_complete(forward, header, n, now);
	  return;
	}
  ...
  if ((forward->flags & FREC_RECURSIVE) && is_referral(header, (size_t)n))
    {
      char ref_zone[MAXDNAME];
      struct referral_server *new_servers;

      ref_zone[0] = '\0';
      referral_zone_name(header, (size_t)n, ref_zone, sizeof(ref_zone));
      {
	unsigned long init_ns_ttl = 0;
	new_servers = parse_referral(header, (size_t)n, ref_zone, &init_ns_ttl);
	...
	if (!new_servers)
	  {
	    ...
	    if (daemon->iterative_async_limit > 0 &&
		resolve_ns_name_async(forward, ns_names, ns_count, ref_zone, now))
	      return;
	...
	if (new_servers)
	  {
	    if (ref_zone[0] != '\0')
	      deleg_cache_store(ref_zone, new_servers, (time_t)init_ns_ttl);

	  forward->referral_servers = new_servers;
	  forward->recursive_iteration = 1;
	  safe_strncpy(forward->recursive_prev_zone, ref_zone, MAXDNAME);

	  if (option_bool(OPT_LOG))
	    my_syslog(LOG_INFO, _("recursive: initial referral for %s from root server"),
		      daemon->namebuff);
      ...
      else
	{
	  ...
	  if (STAT_ISEQUAL(status, STAT_SECURE))
		...
		ede = EDE_DNSSEC_BOGUS;
	      ...
	      if (extract_name(header, (size_t)n, NULL, daemon->namebuff, EXTR_NAME_EXTRACT, 0))
			...
			int have_mark, unsigned int mark, struct server **servp)
{
  ...
  while (1) 
      ...
      if (serv->tcpfd == -1)
	{
	  ...
	  /* Copy connection mark of incoming query to outgoing connection. */
	  if (have_mark)
	    if (setsockopt(serv->tcpfd, SOL_SOCKET, SO_MARK, &mark, sizeof(unsigned int)) == -1)
	      my_syslog(LOG_WARNING, _("failed to set SO_MARK: %s"), strerror(errno));
#endif			  
	  ...
	  tv.tv_usec = 0;
	  if (setsockopt(serv->tcpfd, SOL_SOCKET, SO_SNDTIMEO, &tv, sizeof(tv)) == -1 && option_bool(OPT_LOG))
	    my_syslog(LOG_WARNING, _("failed to set TCP send timeout: %s"), strerror(errno));
	  tv.tv_sec += TCP_TIMEOUT;
	  if (setsockopt(serv->tcpfd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)) == -1 && option_bool(OPT_LOG))
	    my_syslog(LOG_WARNING, _("failed to set TCP receive timeout: %s"), strerror(errno));
#endif
	      ...
	      else
		{
		  ...
		  if ((m = tcp_talk(first, last, start, packet, size, have_mark, mark, &serv)) == 0)
			...
			no_cache_dnssec = 1;
		      ...
		      m = process_reply(header, now, serv, m,
...
static void free_frec(struct frec *f)
{
  ...
  if (f->referral_servers)
    {
      free_referral_servers(f->referral_servers);
      f->referral_servers = NULL;
    }
  f->recursive_iteration = 0;
  f->recursive_prev_zone[0] = '\0';
  if (f->cname_original)
    {
      blockdata_free(f->cname_original);
      f->cname_original = NULL;
    }
  f->cname_original_len = 0;
  f->cname_original_type = 0;
  f->cname_chase_count = 0;
  {
    struct cname_chain *cc = f->cname_chain, *next;
    while (cc)
      {
	next = cc->next;
	free(cc);
  ...
  if (f->async_child)
    {
      f->async_child->async_parent = NULL;
    }
  if (f->async_parent)
    {
      f->async_parent->async_child = NULL;
    }
  f->async_child = NULL;
  f->async_parent = NULL;
  f->async_ns_count = 0;
...
static struct frec *get_new_frec(time_t now, struct server *master, int force)
{
  ...
  for (f = daemon->frec_list, oldest = NULL, target =  NULL, count = 0; f; f = f->next)
    {
      if (!f->sentto && !((f->flags & FREC_RECURSIVE) && f->referral_servers))
	target = f;
      else
	{
#ifdef HAVE_DNSSEC
      ...
      if (f->sentto && difftime(now, f->time) < (double)TIMEOUT && server_samegroup(f->sentto, master))
	count++;
    }
  ...
  if (!force)
      ...
      if (count >= daemon->ftabsize)
	{
      ...
      if (!target && oldest && difftime(now, oldest->time) >= (double)TIMEOUT)
	{ 
