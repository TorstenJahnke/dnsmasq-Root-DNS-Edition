...
#define OPT_RECURSIVE      78
#define OPT_ROOT_PRIMING   79
#define OPT_WIPE_DNSSEC    80
#define OPT_CNAME_FLAT     81
#define OPT_CNAME_MINIM    82
#define OPT_LAST           83
...
#define FREC_RECURSIVE       1024
#define FREC_NS_ASYNC        2048

struct referral_server {
  union mysockaddr addr;
  struct referral_server *next;
};

#define DELEGATION_CACHE_SIZE 4096
#define DELEGATION_MAX_ADDRS  13

struct delegation_entry {
  char zone[MAXDNAME];
  union mysockaddr addrs[DELEGATION_MAX_ADDRS];
  int addr_count;
  time_t expires;
  struct delegation_entry *next;
};

#define MAX_RECURSIVE_ITERATIONS 20

struct cname_chain {
  char source[MAXDNAME];
...
struct frec {
  ...
  char recursive_prev_zone[MAXDNAME];
  struct referral_server *referral_servers;
  struct blockdata *cname_original;
  size_t cname_original_len;
  unsigned short cname_original_type;
  int cname_chase_count;
  struct cname_chain *cname_chain;
  struct frec *async_parent;
  struct frec *async_child;
  char async_ns_zone[MAXDNAME];
...
extern struct daemon {
  ...
  char *root_hints_file;
  char *root_zone_file;
  struct delegation_entry **deleg_cache;
  int deleg_cache_size;
  int deleg_cache_entries;
  int iterative_async_limit;
  int async_ns_in_flight;
...
unsigned char *do_rfc1035_name(unsigned char *p, const char *sval, const char *limit);
...
void my_syslog(int priority, const char *format, ...) ATTRIBUTE_FORMAT(printf, 2, 3);
